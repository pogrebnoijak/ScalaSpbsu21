...
import org.spbsu.mkn.scala.MyGenericList.{ComparatorGList, MyNil, fromSeq, plus, size, sort, sum, turnerInt, turnerStr}
...
class MyGenericListTest extends AnyFunSuite {
  ...
  test("sort") {
    assert(sort(fromSeq(Seq(1,2,3)))(Ordering[Int]) == fromSeq(Seq(3,2,1)))
    assert(sort(fromSeq(Seq(1,2,3)))(Ordering[Int].reverse) == fromSeq(Seq(1,2,3)))
    assert(sort(fromSeq(Seq(25,30,75,42,39,25,8,31,98,12,59,23,7,60,18)))(Ordering[Int].reverse)  ==
      fromSeq(Seq(7,8,12,18,23,25,25,30,31,39,42,59,60,75,98)))
    assert(sort(fromSeq(Seq(5,2,21,2,3,2,5,1,5,2)))(Ordering[Int].reverse) ==
    ...
    assert(sort(fromSeq(Seq(fromSeq(Seq(MyNil, MyNil)), fromSeq(Seq(MyNil, Seq(fromSeq(Seq(1,2)))))))) ==
